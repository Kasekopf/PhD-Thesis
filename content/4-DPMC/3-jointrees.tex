...
Project-join trees have previously been studied in the context of database-query optimization \cite{mcmahan2004projection}.
...
% Project-join trees are related to \emph{variable elimination} on factor graphs \cite{kask2005unifying,bacchus2009solving}, which uses tree decompositions of the primal graph of a factor graph.

% To do this, we separate the algorithm of \cite{dudek2020addmc} into two phases.
...
% Although \cite{dudek2020addmc} considered various heuristics for performing this rearrangement (using bucket elimination \cite{dechter1999bucket} and Bouquet's Method \cite{bouquet1999gestion}), they did not specifically analyze the quality of the rearrangement is isolation from the underlying data structures.
...
% Moreover, \cite{dudek2020addmc} observed that, $\phi$ and $W$ can both be given in factored forms $\phi = \prod_{C_i} C_i$ (\ie, as a set of clauses) and $W = \prod_{x \in X}$ (\ie, as a literal-weight function).
