...
We argue that project-join trees provide a natural formalism to describe execution plans for dynamic-programming algorithms for model counting.
In particular, considering project-join trees as \emph{execution plans} enables us to decompose dynamic-programming algorithms such as the one in \cite{DPV20} into two phases, following the breakdown in Chapter \ref{ch:tensors}: a \emph{planning} phase and an \emph{execution} phase.
