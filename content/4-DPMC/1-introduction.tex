...
In particular, considering project-join trees as execution plans enables us to decompose dynamic-programming algorithms for model counting such as the one in \tool{ADDMC} \cite{DPV20,phan2019weighted} into a 2-phase framework: a \emph{planning} phase, where a project-join tree is determined, and an \emph{execution} phase, where the the determined project-join tree is used to compute the model count.
