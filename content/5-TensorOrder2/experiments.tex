...
All counters are run in Docker images (one for each counter) with Docker 19.03.5. All experiments are run on Google Cloud \tool{n1-standard-8} machines with 8 cores (Intel Haswell, 2.3 GHz) and 30 GB RAM. GPU-based counters are provided an \tool{NVIDIA Tesla V100} GPU (16 GB of onboard RAM) using NVIDIA driver 418.67 and CUDA 10.1.243. For TPU experiments we used v2-8 TPUs, which contain 8 TPU cores and 8 GB of onboard RAM per core.
...
\vspace*{-0.5cm}
\caption{\label{fig:parallel:tpu} The compilation and execution time of performing tensor-network contraction on a CPU vs. on a TPU, based on the max-rank of the contraction tree. \tool{TPU-graph} took more than 1000 seconds in the compilation step when the sliced max-rank is above 17.}
\end{center}
\vspace*{-0.8cm}
\end{figure}

Finally, we examine the feasibility of leveraging a TPU in the execution phase.
...
We consider a tensor network $N$ whose contraction is the number of vertex covers of a randomly-generated cubic graph with 200 vertices \cite{KCMR18}. 
...
We also observe that the compilation time of \pkg{TPU-graph} scales dramatically in sliced max-rank, which is the maximum number of indices of the tensors involved in the computation.
In particular, on tensors with more than 17 dimensions the compilation time of \pkg{TPU-graph} is above 1000 seconds and so the compilation times out.
% Further manual investigations
% This observation explains why all 32 of our initial benchmark set also timed out: all 32 benchmarks involved tensors with more than 25 indices.
% On the other hand, the compilation time of \pkg{CPU8-graph} does not increase as the number of tensor indices increases.
...
Once the compilation bottleneck is improved and tensor networks with larger tensors may be used, the execution time of \pkg{TPU-graph} may outperform \pkg{CPU8-graph}.
...
Unfortunately, as the implementation of the XLA compiler for a TPU is proprietary and available for use only as a black-box tool, we leave an in-depth analysis for future work.
