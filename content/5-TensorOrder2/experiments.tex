...
We implement our changes on top of \tool{TensorOrder} \cite{DDV19} (which implements Algorithm \ref{alg:wmc}) to produce \tool{TensorOrder2}, a new parallel weighted model counter. Implementation details are described in Section \ref{sec:experiments:impl}. All code is available at  \url{https://github.com/vardigroup/TensorOrder}.
...
\tool{TensorOrder2} is primarily implemented in Python 3 as a modified version of \tool{TensorOrder}. We replace portions of the Python code with C++ (\tool{g++} v7.4.0) using Cython 0.29.15 for general speedup, especially in $\func{FactorTree}(\cdot)$.
