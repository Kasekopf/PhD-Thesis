\section{Parallelizing the Execution Phase with Slicing}
...
This was done in Chapter \ref{ch:tensors} using \tool{numpy} \cite{numpy}, and it is straightforward to adjust the implementation to leverage multiple cores with \tool{numpy}, a GPU with \tool{TensorFlow} \cite{ABCCDDDGII16}, and even a TPU with \tool{TensorFlow} or \tool{jax} \cite{jax2018github}.
...
A TPU (tensor processing unit) \cite{JYPPABBBBB17} is a specialized hardware designed for neural network training and inference. %TODO: more on how great TPUs are.
